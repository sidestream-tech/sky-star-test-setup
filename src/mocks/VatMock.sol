...
contract VatMock {
    ...
    function fold(bytes32 i, int rate) external auth {
        Ilk storage ilk = ilks[i];
    ...
    function _sub(uint256 x, int y) internal pure returns (uint z) {
        z = y >= 0 ? x - uint256(y) :x + uint256(-y);
    }

    function _mul(uint x, int y) internal pure returns (int z) {
        z = int(x) * y;
        require(y == 0 || z / y == int(x), "VatMock/mul-overflow");
    }
}
